...
class SignUpForm extends React.Component<any, object> {
    ...
    signup = () => {
        this.props.form.validateFields((err: any, values: userForm): void => {
            if (!err) {
    ...
    checkName = (rule: object, value:string, callback:any) => {
        if(!value) {
            callback('Please input your username!')
        } else {
            request.checkName(value).then((res: AxiosResponse<{uid: number}>) => {
                ...
                <Form>
                    <FormItem label="用户名">
                        {getFieldDecorator('userName', { rules: this.formRules.userName })(
                            <Input placeholder="请输入用户名" onBlur={this.checkName.bind(this)} />
                        )}
                    </FormItem>
                    <FormItem label="密码">
